...
private:
    ...
    void InitializeComponent()
    {
        ...
        btnSortPay->Location = System::Drawing::Point(x + w + 90, y + h + gap);
    ...
    int GetSelectedIndex()
    {
        ...
        if (dgv->SelectedRows == nullptr) return -1;
    ...
    void OnEditClick(System::Object^ sender, System::EventArgs^ e)
    {
        ...
        if (dlg->ShowDialog(this) == System::Windows::Forms::DialogResult::OK)
        {
            try {
                std::string name = ToStdString(dlg->WorkName);
                dept->updateWorkType(idx, name,
                    dlg->BasePay, dlg->BonusPercent);
                RefreshGrid();
            }
            catch (const PayrollException& ex) {
                ShowError(ToSystemString(ex.what()));
    ...
    void OnLoadClick(System::Object^ sender, System::EventArgs^ e)
    {
        ...
        dlg->Filter =
            "CSV/TXT (*.csv;*.txt)|*.csv;*.txt|Все файлы (*.*)|*.*";

        if (dlg->ShowDialog(this) == System::Windows::Forms::DialogResult::OK)
        {
            try {
                dept->loadFromFile(ToStdString(dlg->FileName));
                RefreshGrid();
            }
            catch (const PayrollException& ex) {
                ShowError(ToSystemString(ex.what()));
    ...
    void OnSaveClick(System::Object^ sender, System::EventArgs^ e)
    {
        ...
        {
            try {
                dept->saveToFile(ToStdString(dlg->FileName));
            }
            catch (const PayrollException& ex) {
                ShowError(ToSystemString(ex.what()));
            }
